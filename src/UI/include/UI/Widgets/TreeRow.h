...
// ─────────────────────────────────────────────────────────────────────────────
//  Tree rows — the fixed column slots and chrome of an expandable, zebra-banded
//  hierarchy list. Extracted from the Outliner so every editor that shows a
//  tree looks and behaves identically. Slot order, left → right:
//
//     [active-dot gutter] [chevron slot] [icon slot] [name ……] [trailing]
//
//  Used by the Outliner and by the Colour Usage editor; anything else that
//  needs a hierarchy should build on these rather than re-deriving the metrics.
//
...
namespace UI {
namespace Tree {   // tree-row layout primitives
