...
ImVec4 SafeColor(DesignSystem::Tok t, ImVec4 fallback) {
  try {
    return DesignSystem::DesignSystem::Instance().GetColor(t);
  } catch (...) {
    return fallback;
  }
}
float SafeFloat(DesignSystem::Tok t, float fallback) {
  try {
    return DesignSystem::DesignSystem::Instance().GetFloat(t);
  } catch (...) {
    return fallback;
  }
}
...
// State priority: which cell "owns" a shared edge / draws on top. 3 = active
// (mouse down)   2 = hovered   1 = selected   0 = normal
...
ButtonGroup::Result ButtonGroup::Render() {
  Result result;
  if (cells_.empty() || colW_.empty() || rowH_.empty())
    return result;
  ...
  float radius =
      SafeFloat(DesignSystem::Tok::C_Frame_CornerRadius, 4.0f) * scale;
  float borderSize =
      SafeFloat(DesignSystem::Tok::C_Toggle_BorderWidth, 1.0f) * scale;

  // ── Column / row pixel offsets ──────────────────────────────────────
  std::vector<float> colX(colW_.size() + 1, 0.0f);
  for (size_t i = 0; i < colW_.size(); ++i)
    colX[i + 1] = colX[i] + colW_[i];
  std::vector<float> rowY(rowH_.size() + 1, 0.0f);
  for (size_t i = 0; i < rowH_.size(); ++i)
    rowY[i + 1] = rowY[i] + rowH_[i];
  ...
  for (size_t i = 0; i < cells_.size(); ++i) {
    ...
    bool clicked = ImGui::Button("##cell", ImVec2(mx.x - mn.x, mx.y - mn.y));
    bool hovered = ImGui::IsItemHovered();
    bool active = ImGui::IsItemActive();
    ImGui::EndDisabled();
    if (clicked && c.enabled)
      result.clickedIndex = static_cast<int>(i);
    ...
    rc[i] = {
        ...
        c.enabled && hovered,
        c.enabled && active,
        StatePriority(c.selected, c.enabled && hovered, c.enabled && active)};
  }
  ...
  auto cellFill = [&](const Resolved &r) -> ImVec4 {
    ...
    if (r.selected) {
      if (r.active)
        return Lerp(activeBg, ImVec4(0, 0, 0, 1), 0.10f);
      if (r.hovered)
        return Lerp(activeBg, ImVec4(1, 1, 1, 1), 0.08f);
      return activeBg;
    }
    if (r.active)
      return Lerp(hoverBg, activeBg, 0.25f);
    if (r.hovered)
      return hoverBg;
    return baseBg;
  };
  auto cellBorder = [&](const Resolved &r) -> ImVec4 {
    if (!r.enabled)
      return Lerp(border, disabledBg, 0.55f);
  ...
  auto cornerFlagsFor = [&](const ImVec2 &mn, const ImVec2 &mx) -> ImDrawFlags {
    ...
    if (L && T)
      f |= ImDrawFlags_RoundCornersTopLeft;
    if (R && T)
      f |= ImDrawFlags_RoundCornersTopRight;
    if (R && B)
      f |= ImDrawFlags_RoundCornersBottomRight;
    if (L && B)
      f |= ImDrawFlags_RoundCornersBottomLeft;
  ...
  for (const auto &r : rc) {
    dl->AddRectFilled(r.mn, r.mx, ImGui::ColorConvertFloat4ToU32(cellFill(r)),
  ...
  for (size_t a = 0; a < rc.size(); ++a) {
    for (size_t b = a + 1; b < rc.size(); ++b) {
      float x;
      if (eq(rc[a].mx.x, rc[b].mn.x))
        x = rc[a].mx.x;
      else if (eq(rc[b].mx.x, rc[a].mn.x))
        x = rc[b].mx.x;
      else
        continue;
      ...
      if (y1 <= y0)
        continue;
      const Resolved &owner = rc[a].priority >= rc[b].priority ? rc[a] : rc[b];
      dl->AddLine(ImVec2(x, y0), ImVec2(x, y1), borderU(owner), borderSize);
    }
  }
  ...
  for (size_t a = 0; a < rc.size(); ++a) {
    for (size_t b = a + 1; b < rc.size(); ++b) {
      float y;
      if (eq(rc[a].mx.y, rc[b].mn.y))
        y = rc[a].mx.y;
      else if (eq(rc[b].mx.y, rc[a].mn.y))
        y = rc[b].mx.y;
      else
        continue;
      ...
      if (x1 <= x0)
        continue;
      const Resolved &owner = rc[a].priority >= rc[b].priority ? rc[a] : rc[b];
      dl->AddLine(ImVec2(x0, y), ImVec2(x1, y), borderU(owner), borderSize);
    }
  }
  ...
  const float iconSz =
      SafeFloat(DesignSystem::Tok::C_Dropdown_IconSize, 16.0f) * scale;
  const float leftPad =
      SafeFloat(DesignSystem::Tok::C_Menu_ItemPaddingX, 6.0f) * scale;
  ...
  for (size_t i = 0; i < cells_.size(); ++i) {
    ...
    while (*lblEnd && !(lblEnd[0] == '#' && lblEnd[1] == '#'))
      ++lblEnd;
    ImVec2 ts = ImGui::CalcTextSize(lbl, lblEnd);
    const bool hasIcon = !c.icon.empty();
    ImVec4 tc = !c.enabled ? disabledTx : (c.selected ? activeTx : text);
