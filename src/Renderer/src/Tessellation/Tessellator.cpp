...
static inline void PushVert(Mesh& m, Vec2 p, const Color& c) {
    m.vertices.push_back(Vertex{p.x, p.y, c.r, c.g, c.b, c.a});
}
static inline void PushTri(Mesh& m, Vec2 a, Vec2 b, Vec2 c, const Color& col) {
...
static float gQualityPerUnit = 4.0f;    // ≈ 4 segments / doc-unit of arc
...
static float gDetailScale = 1.0f;
...
static void FlattenEdge(const Node& a, const Node& b, float /*zoom*/,
                        std::vector<Vec2>& out) {
...
static void EvalBSpline(const std::vector<Node>& ctrl, int order, bool closed,
