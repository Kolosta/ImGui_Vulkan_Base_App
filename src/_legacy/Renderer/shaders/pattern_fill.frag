...
// Procedural fill-pattern fragment shader. The motif (dots / lines / triangles /
// random-dots / grid / cross-hatch) is computed from the DOCUMENT-space fragment
// position + pattern parameters — no per-element CPU geometry. The surface stencil
// clips it to the contour, so editing spacing/size/angle/offset/contour costs zero
// CPU work. Anti-aliasing is automatic: fwidth() is in screen pixels (it includes
// zoom·unitScale·SSAA), so a 1px-soft edge falls out of smoothstep on doc-units.
...
// Murmurish hash + signed jitter, ported VERBATIM from the CPU generator so the
// scatter is pixel-identical to the previous (instanced) look.
...
// Signed distance from `p` to the inside of triangle (A,B,C) (negative inside).
float triInside(vec2 p, vec2 A, vec2 B, vec2 C) {
    // Per-edge half-plane distance; CCW or CW handled by sign(area).
    ...
    // Edge lengths to normalise the cross products into true distances.
    float l0 = length(B-A), l1 = length(C-B), l2 = length(A-C);
    return max(max(-d0/max(l0,1e-6), -d1/max(l1,1e-6)), -d2/max(l2,1e-6)); // >0 outside
}
...
void main() {
    ...
    if (k == 1 || k == 4) {                      // Dots / RandomDots
        ...
        int ring = int(ceil((0.5*sp + sz) / sp));   // cells whose triangle may reach here
        vec2 cell = floor(q / sp + 0.5);
        ...
        for (int dx = -ring; dx <= ring; ++dx) {
            ...
            mat2 Rt = mat2(c2, s2, -s2, c2);     // inverse of the instance rotation
            vec2 f = Rt * (q - c);
            float d = triInside(f, A, B, C);     // >0 outside
            cov = max(cov, smoothstep(aa*0.5, -aa*0.5, d));
        }
    } else {                                     // Lines (2) / Grid (5) / CrossHatch (6)
        float halfw = sz * 0.5;
        // First line set along local +x (across = q.y).
        ...
        cov = covV;
        if (k == 5 || k == 6) {                  // second perpendicular set (along +y)
